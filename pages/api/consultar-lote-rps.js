...
export default async function handler(req, res) {
  ...
  if (!req.body || !req.body.Prestador || !req.body.Protocolo) {
    return res.status(400).json({
      success: false,
      message: 'Dados inválidos',
      error: 'O corpo da requisição deve conter os dados do Prestador e Protocolo'
    });
  }
  ...
  const xml = buildConsultarLoteRpsXml(req.body);
